...
// ─── PRELOADER ───────────────────────────────────────────────
// Tied to actual asset loading — not a fake timer
const preloader = document.getElementById('preloader');
const preloaderCount = document.querySelector('.preloader-count');
let fakeProgress = 0;
let preloaderDone = false;

// Fake progress runs until real load fires
const fakeInterval = setInterval(() => {
  if (fakeProgress < 85) {
    fakeProgress += Math.random() * 8 + 2;
    if (fakeProgress > 85) fakeProgress = 85;
    if (preloaderCount) preloaderCount.textContent = Math.floor(fakeProgress) + '%';
  }
}, 100);
...
// Real event — fires when ALL assets (fonts, scripts, images) are loaded
if (document.readyState === 'complete') {
  hidePreloader();
} else {
  window.addEventListener('load', hidePreloader);
  // Fallback — never hang longer than 5 seconds
...
// ─── HAMBURGER ────────────────────────────────────────────────
// Uses CSS class toggle — not fragile inline styles
...
if (portfolioWrap) {
  ...
  portfolioWrap.addEventListener('mousedown', e => {
    ...
    portfolioWrap.style.cursor = 'grabbing';
  });
  portfolioWrap.addEventListener('mouseleave', () => { isDown = false; portfolioWrap.style.cursor = 'grab'; });
  portfolioWrap.addEventListener('mouseup', () => { isDown = false; portfolioWrap.style.cursor = 'grab'; });
  portfolioWrap.addEventListener('mousemove', e => {
    if (!isDown) return;
    e.preventDefault();
    const x = e.pageX - portfolioWrap.offsetLeft;
    portfolioWrap.scrollLeft = scrollLeft - (x - startX) * 1.5;
  });
  
  // Touch swipe (mobile) - native CSS scroll handles this
  // but we add momentum feedback
  portfolioWrap.addEventListener('touchstart', e => {
    startX = e.touches[0].pageX;
    scrollLeft = portfolioWrap.scrollLeft;
  }, { passive: true });
  portfolioWrap.addEventListener('touchmove', e => {
    const x = e.touches[0].pageX;
    portfolioWrap.scrollLeft = scrollLeft - (x - startX);
  }, { passive: true });
}
...
function initHeroCanvas() {
  ...
  function getCanvasSize() {
    return Math.min(canvas.parentElement?.offsetWidth || 500, 600);
  }
  ...
  // Responsive resize for hero canvas
  const heroResizeObs = new ResizeObserver(() => {
    const s = getCanvasSize();
    renderer.setSize(s, s);
    camera.aspect = 1;
    camera.updateProjectionMatrix();
  });
  if (canvas.parentElement) heroResizeObs.observe(canvas.parentElement);
}
...
// initAnimations is called by hidePreloader above
